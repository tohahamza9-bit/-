...
class Deal(BaseModel):
    ...
    # 🔴 وُلدت من **إعادة استخدام مرجع** (X1242): رسالةٌ حملت مرجعًا مستعمَلًا بمحتوى مختلف
    #   فأُنشئت لها صفقة جديدة بدل التجاهل. غالبًا رسالةٌ **ثانية** (تسوية/خزينة) انفصلت عن
    #   أُولاها. تُستبعَد من ترشّح التكملات **عديمة المرجع** (§7.3): محتواها كاملٌ سلفًا،
    #   وجلوسها في الطابور يكسر تقابل ١:١ الذي تفترضه FIFO فتنزاح الروابط بواحد
    #   (عائلة X850‑X853، ودفعة 2026-07-20). المرجع الصريح ما زال يبلغها عبر bind_by_reference.
    born_from_ref_reuse: bool = False
    # 🔴 حارس محاولة التكملة (العرض 2-A): كم مرّةً حاولت رسالةٌ ثانية عديمة المرجع أن تُكمِل هذه
    #   الصفقة، ومتى آخر محاولة. الغرض: **استبعاد ذرّيّ** لصفقةٍ جرت محاولةُ إكمالها من إعادة
    #   الترشّح تحت السباق (كيرنل مكرّر) أو إعادة المعالجة — فلا تُبتلَع تكملةٌ ثانية بصفقةٍ
    #   خاطئة لأنّها الأقدم. يُزاد ذرّيًّا عبر DealRepo.claim_completion_attempt قبل الدمج.
    completion_attempt_count: int = 0
    completion_attempted_at: Optional[datetime] = None
...
class DetectionConfig(BaseModel):
    ...
    # انتظار الرسالة الثانية قبل نداء الذكاء. 3ث لا 8: التكملة تصل عادةً خلال ثانية أو
    # ثانيتين من نفس الموظّف (رُصد في دفعة 13:52 الحيّة: كل تكملة تلت أصلها بثانية)،
    # و8ث كانت تأخيرًا محسوسًا للمُرسِلين بلا مقابل. قابل للضبط حيًّا من اللوحة.
    ai_pair_wait_seconds: float = 3.0
