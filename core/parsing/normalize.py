...
# «جني» (بلا هاء) إملاءٌ شائع أسقط حوالتين في يوم واحد (2026-07-20): X1478 «مبلغ40.000 جني م»
#   فشل استخراج مبلغها فصُنّفت «بلا بنية» ولم تُنشَأ لها صفقة؛ وX1567 «ارجو تحويل 5800 جني».
#   («جنية» بالتاء المربوطة لا تحتاج إدخالًا: normalize_ar يردّها «جنيه» سلفًا.)
_EGP_TOKENS = {"ج", "جم", "جنيه", "جني", "مصري", "دم", "دمصري", "دمصرى"}   # «دم»/«دمصري» ملتصقة شائعة (§3.4)
...
def normalize_payment(s: Optional[str]) -> Optional[str]:
    ...
    «واتساب/واتس اب» = فودافون (تأكيد المالك 2026-07-21، X1702)."""
    if not s:
        return None
...
# ── التباس «ألف/آلاف/مليون» (§3.5) 🔴 ─────────────────────────────────────────
# «32 ألف» كانت تُقرأ 32 (الكلمة تُهمَل) بدل 32000 — سبّبت عملية شبح بمبلغ خاطئ. القاعدة: عدد
# صحيح (بلا نقطة/فاصلة/رقم قبله) + «ألف/آلاف/مليون/ملايين» ككلمة تامّة → يُضرَب في مُعامِلها.
# حدّ الكلمة (?![ء-ي]) يمنع إشعال أسماء تبدأ بـ«الف» («الفرجاني/الفوني») أو «مليون»؛ «1.000 ألف»/
# «10.000»/«32,000» تمرّ (فاصل قبله). «مليون» أُضيف (2026-07-23) كي يُوحَّد مع حارس _num_in_text.
_AMOUNT_WORD_MULT = {
    "ألف": 1000, "الف": 1000, "آلاف": 1000, "الاف": 1000,
    "مليون": 1_000_000, "ملايين": 1_000_000,
}
_ALF_EXPAND_RE = re.compile(
    r"(?<![.,،٫٬\d])(\d+)\s*(ألف|الف|آلاف|الاف|مليون|ملايين)(?![ء-ي])")


def expand_alf_amounts(raw: Optional[str]) -> tuple[str, list[dict]]:
    """يوسّع «عدد + ألف/آلاف/مليون» → عدد×(1000/1e6) داخل النصّ (§3.5). يُرجع (النصّ المُوسَّع، سجلّ
    التوسّعات [{original, value}]؛ فارغ إن لا توسّع). يُطبَّق على نصّ بأرقام لاتينية (بعد normalize_digits)."""
    if not raw:
        return raw or "", []
    expansions: list[dict] = []

    def _repl(m: "re.Match[str]") -> str:
        value = int(m.group(1)) * _AMOUNT_WORD_MULT[m.group(2)]
