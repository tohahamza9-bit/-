...
def is_stable(raw: RawMessage, now: datetime) -> bool:
    ...
    """
    # 🔴 استرجاع وقت العطل (§12): أي رسالة عمرها ≥ STABILIZE_MAX (90s) استقرّت قطعًا — نافذة
    #    تعديل «الحرف» انقضت. تُعامَل مستقرّةً فورًا (True) بغضّ النظر عن محتواها، فتُعالَج عند
    #    الإقلاع لا تُترَك عالقة. (مكافئ منطقيًّا للفروع أدناه، لكنه يُصرّح بالنيّة صراحةً.)
