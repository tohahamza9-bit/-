"""
هجرة صغيرة آمنة (idempotent): تضمن أن خزينة «خصم 1%» بكود 72 (الافتراضية لـ SI مع مورد)، EGP،
sell_and_buy، نشطة — وتحذف أي خزينة أخرى بكود 72 (ازدواج لنفس الحساب، قرار صاحب العمل).
سببها: seed_if_empty يبذر المجموعة الفارغة فقط، فقاعدة بيانات مأهولة لا تتحدّث تلقائيًا
(§4، ملحق ب-3). (رُجِع الكود من 85 المؤقّت إلى 72.)
...
TREASURY = {
    "name": "خصم 1%",
    "code": "72",   # الافتراضية لـ SI مع مورد (كانت 85 مؤقّتًا)
    "currency": "EGP",
    "type": "sell_and_buy",
...
def main() -> None:
    ...
    print("قبل الهجرة — سجلات كود 72/85 أو «خصم 1%»:")
    for d in db.treasuries.find({"$or": [{"code": code_in}, {"code": {"$in": ["85", 85]}},
                                         {"name": "خصم 1%"}]}):
        print(f"  name={d.get('name')!r} code={d.get('code')!r} active={d.get('active')}")

    # (1) احذف أي ازدواج بكود 72 لخزينة **غير** «خصم 1%» (نفس الحساب — قرار صاحب العمل).
    dup = db.treasuries.delete_many({"code": code_in, "name": {"$ne": "خصم 1%"}})
    print(f"\nحُذف من ازدواج كود 72 (غير «خصم 1%»): {dup.deleted_count}")

    # (2) ثبّت «خصم 1%» بالاسم (unambiguous) — كود 72، EGP، sell_and_buy، نشطة.
    #     التحديث بالاسم يَنقل السجل من كوده القديم (85) إلى 72 في مكانه.
    ...
    left85 = db.treasuries.count_documents({"code": {"$in": ["85", 85]}})
    print(f"سجلات كود 85 المتبقّية: {left85}")
    if len(after) != 1:
        print("⚠️ عدد سجلات كود 72 ليس 1 — راجع يدويًا.")
