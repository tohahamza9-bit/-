...
async def test_burst_refless_continuations_reach_true_owner(db):
    ...
    كان هذا xfail(strict) حتى 2026-07-20: التكملات كانت تنزاح بواحد لأنّ الصفقات المولودة
    من مرجعٍ مكرَّر (k4/k6) تجلس في الطابور شَرَكًا فتكسر تقابل ١:١ الذي تفترضه FIFO.
    الحسم: وسمُ الشَّرَك عند الإنشاء (born_from_ref_reuse) واستبعاده من ترشّح عديمي المرجع.
    حتميّ بالكامل — لا يعتمد على الذكاء ولا على حالة الصفقة السابقة."""
    await _run(_pipe(db), BURST)
    pairs = await _pairs(db)
    wrong = {
